  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      <section className="relative overflow-hidden">
        <div className="absolute inset-x-0 top-0 h-72 bg-blue-600/20 blur-3xl" />
        <div className="relative mx-auto max-w-6xl px-6 py-20 sm:px-10 lg:px-12">
          <div className="grid gap-16 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div className="space-y-8">
              <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-1 text-sm text-slate-200 shadow-sm shadow-blue-500/10">
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <Link href="/register" className="inline-flex items-center justify-center rounded-2xl bg-blue-500 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/20 transition hover:bg-blue-400">
                  Create account
                </Link>
      ...
      <section id="listings" className="border-t border-white/10 bg-slate-900/40">
        <div className="mx-auto max-w-6xl px-6 py-16 sm:px-10 lg:px-12">
          <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
            ...
            <Link href="/listings" className="text-sm font-semibold text-blue-300 hover:text-blue-200 transition">
              See all listings →
            </Link>
          </div>
          ...
          {listingsLoading ? (
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 px-6 py-10 text-center">
              ...
              <Link href="/listings/new" className="inline-flex items-center justify-center rounded-xl bg-blue-500 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-400">
                Post a listing
              </Link>
            </div>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {listings.map(l => (
                <Link key={l.id} href={`/listings/${l.id}`}
                  className="group rounded-2xl border border-white/10 bg-white/5 overflow-hidden transition hover:border-blue-400/40 hover:bg-white/[0.07]">
                  ...
                  <div className="p-4">
                    <span className="text-[11px] font-medium text-blue-300 uppercase tracking-wide">{TYPE_LABELS[l.type]}</span>
                    <p className="font-semibold text-white mt-0.5 truncate group-hover:text-blue-200 transition">{l.title}</p>
